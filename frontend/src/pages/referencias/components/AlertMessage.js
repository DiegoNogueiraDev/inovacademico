/**
 * Componente para exibir mensagens de alerta
 */

const AlertMessage = ({ message, showAlert }) => {
  if (!showAlert || !message.text) return null;

  let alertClass = 'bg-gray-100 border-gray-500 text-gray-700';
  let icon = null;

  // Define as classes e ícones com base no tipo de mensagem
  switch (message.type) {
    case 'success':
      alertClass = 'bg-green-100 border-green-500 text-green-700';
      icon = (
        <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"></path>
        </svg>
      );
      break;
    case 'error':
      alertClass = 'bg-red-100 border-red-500 text-red-700';
      icon = (
        <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
          <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clipRule="evenodd"></path>
        </svg>
      );
      break;
    case 'warning':
      alertClass = 'bg-yellow-100 border-yellow-500 text-yellow-700';
      icon = (
        <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
          <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd"></path>
        </svg>
      );
      break;
    case 'info':
      alertClass = 'bg-blue-100 border-blue-500 text-blue-700';
      icon = (
        <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
          <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2h.01a1 1 0 000-2H9z" clipRule="evenodd"></path>
        </svg>
      );
      break;
    default:
      break;
  }

  return (
    <div className={`p-4 mb-4 rounded-md border-l-4 ${alertClass} flex items-center`} role="alert">
      {icon}
      <div>
        {message.text}
      </div>
    </div>
  );
};

export default AlertMessage; 